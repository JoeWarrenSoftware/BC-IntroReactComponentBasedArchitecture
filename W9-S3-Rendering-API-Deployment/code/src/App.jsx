  ...
  return (
    <div style={{ padding: '20px', maxWidth: '600px', margin: 'auto' }}>
      ...
      <button onClick={handleSearch} style={{ padding: '10px', marginTop: '10px', width: '100%' }}>
        ...
        { error!=null && <div>There is an Error with the request. Sorry! Error is: {error}</div>}
        ...
        { catFacts.length > 0 ? 
          ...
          <ul>{catFacts.map((fact, index) => (<li key={index}>{fact.fact}</li>))}</ul>
          </div> 
          : 
          <h2>Data will be shown here when available...</h2>}
        </div>
